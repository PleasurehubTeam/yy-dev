...
description: Use when generating comprehensive EARS-format requirements for a specification
---
...
1. **Load Context**:
   ...
   - Read `.yy-dev/specs/$1/requirements.md` for project description
   - **Load ALL steering context**: Read entire `.yy-dev/steering/` directory including:
     - Default files: `structure.md`, `tech.md`, `product.md`
     - All custom steering files
     - This provides complete project memory and context

2. **Read Guidelines**:
   - Read `${CLAUDE_PLUGIN_ROOT}/templates/rules/ears-format.md` for EARS syntax rules
   - Read `${CLAUDE_PLUGIN_ROOT}/templates/specs/requirements.md` for document structure
...
- Review generated requirements at `.yy-dev/specs/$1/requirements.md`
- **Optional Gap Analysis** (for existing codebases):
  - Run `/yy:validate-gap $1` to analyze implementation gap with current code
  - Recommended for brownfield projects; skip for greenfield
- Then `/yy:spec-design $1 -y` to proceed to design phase

**If Modifications Needed**:
- Provide feedback and re-run `/yy:spec-requirements $1`
